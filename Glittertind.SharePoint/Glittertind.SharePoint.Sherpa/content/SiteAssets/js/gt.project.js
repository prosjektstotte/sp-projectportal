    ...
    ExecuteOrDelayUntilScriptLoaded(function () {
        ...
        ctx.executeQueryAsync(function () {
            ...
            if (_spPageContextInfo.serverRequestPath.endsWith(welcomePage)) {
                ...
                GT.jQuery(selector).html('<p>Information about the project is only available from <a href="../' + welcomePage + '">the frontpage</a></p>').show();
            }
...
GT.Project.ChangeProjectPhase = function () {
    ...
    currentPhasePromise.done(function (term) {
        ...
        if (safeTerm != "" && safeTerm.get_label != undefined) {
            ...
            GT.jQuery.when(
                GT.Project.ChangeQueryOfListViewOnPage(safeTerm.get_label(), "Documents", "SitePages/Frontpage.aspx"),
                GT.Project.ChangeQueryOfListViewOnPage(safeTerm.get_label(), "Tasks", "SitePages/Frontpage.aspx"),
                GT.Project.ChangeQueryOfListViewOnPage(safeTerm.get_label(), "Uncertainties", "SitePages/Frontpage.aspx"),
                GT.Project.SetMetaDataDefaultsForLib("Documents", "GtProjectPhase", safeTerm)
            ).then(function () {
                deferred.resolve();
            });
        }
            ...
            camlArray.push(
                "<Where>",
                    "<Or>",
                        ...
                        "<Eq>",
                            "<FieldRef Name='GtProjectPhase' />",
                            "<Value Type='TaxonomyFieldType'>Several phases</Value>",
                        "</Eq>",
                    "</Or>",
                "</Where>");
...
GT.jQuery(function () {
    GT.Project.FixProjectInfoBox();
});
GT.Project.FixProjectInfoBox = function () {
    GT.jQuery(".GtIndicator").each(function () {
        var $fieldRow = GT.jQuery("." + GT.jQuery(this).data("statustype"));
        var fieldValue = $fieldRow.find("td.fieldValue").text().trim();
        GT.jQuery(this).attr("data-status", fieldValue);
        $fieldRow.hide()
    });
};
    ...
    context.executeQueryAsync(Function.createDelegate(this, function () {
        ...
        context.executeQueryAsync(Function.createDelegate(this, function () {
            ...
            while (termEnumerator.moveNext()) {
                ...
                var showOnFrontpage = (currentTerm.get_localCustomProperties()["ShowOnFrontpage"] == "true");
                if (showOnFrontpage) {
...
GT.Project.GetPhaseLogoMarkup = function (phaseName, selected, wrapInListItemMarkup, linkToDocumentLibrary, index) {
    ...
    if (linkToDocumentLibrary)
        markup = '<a href="../Documents/Forms/AllItems.aspx?FilterField1=GtProjectPhase&FilterValue1=' + phaseDisplayName + '">' + markup + '</a>';
...
GT.Project.PhaseForm.CheckList.render = function () {
    ...
    promise.done(function (items) {
        ...
        outHtml.push('<div id="gtchecklist">',
                        '<h2 class="ms-h2">Phase Checklist</h2>',
                        '<ul>');
...
GT.Project.PhaseForm.CheckList.getData = function () {
    ...
    .then(function (phase) {
        var defer = GT.jQuery.Deferred();
        var ctx = new SP.ClientContext.get_current();
        var list = ctx.get_web().get_lists().getByTitle('Phase Checklist');
...
GT.Project.PhaseForm.CheckList.CheckListItem = function (title, id, status) {
    ...
    self.get_editItemUrl = function (sourceUrl) {
        var editElmLink = _spPageContextInfo.webServerRelativeUrl + "/Lists/PhaseChecklist/EditForm.aspx?ID=" + self.Id;
        if (sourceUrl) {
...
GT.Project.CalendarForm.RenderRelatedLogElements = function () {
    ...
    promise.done(function (items) {
        ...
        outHtml.push('<div id="gtloglist">',
                        '<h2 class="ms-h2">Elements from the log</h2>',
                        '<table><tbody>',
                        '<tr><th>Title</th>',
                        '<th>Description</th>',
                        '<th>Log Element</th>',
                        '<th>Reported by</th></tr>');
        for (var i = 0; i < items.length; i++) {
...
GT.Project.CalendarForm.getData = function () {
    ...
    .then(function (fieldValue) {
        var defer = GT.jQuery.Deferred();
        var ctx = new SP.ClientContext.get_current();
        var list = ctx.get_web().get_lists().getByTitle('Project Log');
        var camlQuery = new SP.CamlQuery();
...
GT.Project.CalendarForm.LogElement = function (title, id, description, type, reportedBy) {
    ...
    self.get_viewItemUrl = function (sourceUrl) {
        var editElmLink = _spPageContextInfo.webServerRelativeUrl + "/Lists/ProjectLog/DispForm.aspx?ID=" + self.Id;
        if (sourceUrl) {
            editElmLink += "&Source=" + encodeURIComponent(sourceUrl);
        }
        return editElmLink;
...
GT.Project.Model.GetStatusCssClass = function (status) {
    return status ? status.toLowerCase().split(" ").join("-") : 'status-unknown';
};
        ...
        var get_pageProperties = function (pagedWebs) {
            ...
            clientContext.executeQueryAsync(function () {
                ...
                for (var i = 0; i < pagedWebs.length; i++) {
                    ...
                    var welcomePageUrl = web.get_serverRelativeUrl() + "/SitePages/Frontpage.aspx";
